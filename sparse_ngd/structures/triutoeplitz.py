...
class TriuToeplitzMatrix(StructuredMatrix):
    """Upper-triangular Toeplitz-structured matrix implemented in the ``StructuredMatrix`` interface.
    ...
    def __init__(self, row: Tensor) -> None:
    ...
    def from_dense(cls, mat: Tensor) -> TriuToeplitzMatrix:
        ...
        Args:
            mat: A dense and symmetric square matrix which will be approximated by a ``TriuToeplitzMatrix``.
        ...
        digitized_tensor = torch.as_tensor(digitized) #using cupy instead of numpy to avoid a cpu-to-gpu call
        result = torch.bincount(digitized_tensor, x.view(-1))

        row = result [ range(dim) ].flip(0) + result [ (dim-1): ]
        row.div_( (1.0 + torch.Tensor(range(dim)) ).flip(0) )
        row[0] = row[0]/2.0
